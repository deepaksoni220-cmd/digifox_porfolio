...
(function () {
  ...
  function initLenis() {
    ...
    lenis = new Lenis({
      ...
      smoothWheel: true,
      touchMultiplier: 1.0   // reduced from 1.6 — prevents over-scrolling past pinned mobile sections
    });
    if (hasGSAP) {
      // Critical: tell ScrollTrigger to read scroll position from Lenis,
      // otherwise pinned scrubs never trigger (Lenis intercepts native scroll)
  ...
  Scrubber.prototype.tick = function () {
    ...
    this.current += d * 0.18;              // scrub:1 feel — lerp, never raw
  ...
  /* ─────────────────────────────────────────────
     ...
     ───────────────────────────────────────────── */
  var HERO_IMAGES = [
    './assets/img/vanta-veiled.jpg',
    './assets/img/vanta-hero.jpg',
    './assets/img/vanta-desert.jpg',
    './assets/img/vanta-silhouette.jpg',
    './assets/img/vanta-wheel.jpg',
    './assets/img/vanta-cabin.jpg'
  ];
  var STATUS = [
    'CALIBRATING STUDIO LIGHTS',
    'PRESSURISING HYDROGEN LINE',
    'WARMING CARBON DISCS',
    'LIFTING THE SHEET'
  ];
  ...
  function heroIntro() {
    ...
    var tl = gsap.timeline({
      ...
      paused: true,
      onComplete: function () {
        // Safety net: clear all GSAP-set inline transforms so text
        // is always in its natural, visible CSS position after intro.
        gsap.set(chars.length ? chars : '.display--hero .line',
    ...
    if (chars.length) {
      // immediateRender: false — do NOT apply the FROM state until play() is called.
      // Without this, GSAP sets yPercent:118 instantly (hiding chars behind overflow:hidden)
      // and if play() is delayed (e.g. Lenis/ScrollTrigger setup on mobile), text stays invisible.
      tl.from(chars, { yPercent: 118, duration: 1.05, stagger: 0.018, immediateRender: false }, 0);
    } else {
      tl.from('.display--hero .line', { yPercent: 110, duration: 1, stagger: 0.08, immediateRender: false }, 0);
    }
    tl.from('.reveal-copy .kicker', { opacity: 0, y: 14, duration: 0.7, immediateRender: false }, 0.15)
      .from('.reveal-copy .lede',   { opacity: 0, y: 16, duration: 0.8, immediateRender: false }, 0.5)
      .from('.edge-stat',           { opacity: 0, y: 12, duration: 0.7, stagger: 0.09, immediateRender: false }, 0.35)
      .from('.reveal-trio span',    { opacity: 0, x: 18, duration: 0.6, stagger: 0.08, immediateRender: false }, 0.55)
      .from('.scroll-cue',          { opacity: 0, duration: 0.6, immediateRender: false }, 0.9)
      .from('.stage-bars i',        { scaleX: 0, opacity: 0, duration: 1.2, stagger: 0.12, immediateRender: false }, 0)
  ...
  function buildScroll() {
    ...
    mm.add('(min-width: 901px) and (prefers-reduced-motion: no-preference)', function () {
      ...
      revealTL
        ...
        .to('.stage-bars', { opacity: 0.1, duration: 0.4, ease: 'none' }, 0.1)
    ...
    /* ══ MOBILE — pinned scrubs (shorter than desktop) ══ */
    mm.add('(max-width: 900px) and (prefers-reduced-motion: no-preference)', function () {
      var kills = [];

      // hero: pin the stage so user has 160% of viewport to complete the veil lift
      ...
      // drive: pin the stage with 180% scroll room
      ...
      var t3 = gsap.from('.gcard', {
        ...
        scrollTrigger: { trigger: '#driveStage', start: 'top 30%', once: true }
      });
      kills.push(t3);
  ...
  window.addEventListener('load', function () {
    if (hasGSAP) {
      // Extra delayed refresh ensures fonts + images are laid out correctly
      ScrollTrigger.refresh();
